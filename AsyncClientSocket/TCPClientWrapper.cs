...
namespace Elections.Ticker.GraphicsIntf.ClientSocket
{
    /// <summary>
    ...
    public class TCPClientWrapper : IDisposable
    {
        ...
        public delegate void delDataReceived(TCPClientWrapper sender, object data);
        public event delDataReceived DataReceived;
        public delegate void delConnectionStatusChanged(TCPClientWrapper sender, ConnectionStatus status);
        ...
        public TCPClientWrapper(IPAddress ip, int port, bool autoreconnect = true)
        {
            this._IP = ip;
            this._Port = port;
        ...
        protected virtual void Dispose(bool disposing)
        {
            lock (SyncLock)
            {
                ...
                if (!this.disposed)
                {
                    ...
                    tmrConnectTimeout.Enabled = false;
                    tmrReceiveTimeout.Enabled = false;
                    tmrSendTimeout.Enabled = false;              
                
                    ////Detach public event subscriptions
                    //if (DataReceived != null)
                    //{
                    //    Delegate[] DataReceivedList = DataReceived.GetInvocationList();
                    //    foreach (Delegate d in DataReceivedList)
                    //        DataReceived -= (d as delDataReceived);
                    //    DataReceived = null;
                    //}

                    //if (ConnectionStatusChanged != null)
                    //{
                    //    Delegate[] ConnectionStatusChangedList = ConnectionStatusChanged.GetInvocationList();
                    //    foreach (Delegate d in ConnectionStatusChangedList)
                    //        ConnectionStatusChanged -= (d as delConnectionStatusChanged);
        ...
        {
            var r = result.AsyncState as TCPClientWrapper;
            if (r == null)
